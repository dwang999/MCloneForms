...
namespace MCloneForms {
	...
	/// <summary>
	/// Summary for IncomesUserControl
	/// </summary>
	public ref class IncomesUserControl : public System::Windows::Forms::UserControl
	{
	private:
	...
	public:
		IncomesUserControl(void): incomeController(nullptr), DIVIDER_HEIGHT(4)
		{
			InitializeComponent();
		}

		void Init(IncomeController* tempController)
		{
			incomeController = tempController;
			// Initialize Form Controls		


			// Initiallize member variables			
			if (incomeController -> backupExists())
			{
				if (FormHelper::showYesNoMessage("MCloneForm Load Backup", "MCloneForm did not close properly. Do you wish to load Incomes from previous session?"))
				{
					incomeController -> loadBackup();
					saveButton -> Enabled = true;
				}
				else
				{
					incomeController -> load();
					incomeController -> deleteBackup();
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->dataGridView->Columns->AddRange(gcnew cli::array< System::Windows::Forms::DataGridViewColumn^  >(3) {this->ID, this->EffectiveDate, 
				this->Amount});
			...
			this->dataGridView->CellContentClick += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &IncomesUserControl::dataGridView_CellContentClick);
			this->dataGridView->CellValueChanged += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &IncomesUserControl::dataGridView_CellValueChanged);
			this->dataGridView->ColumnHeaderMouseClick += gcnew System::Windows::Forms::DataGridViewCellMouseEventHandler(this, &IncomesUserControl::dataGridView_ColumnHeaderMouseClick);
			this->dataGridView->SortCompare += gcnew System::Windows::Forms::DataGridViewSortCompareEventHandler(this, &IncomesUserControl::dataGridView_SortCompare);
			// 
			// editButton
			// 
			...
			this->editButton->Click += gcnew System::EventHandler(this, &IncomesUserControl::editButton_Click);
			// 
			// addButton
			// 
			...
			this->addButton->Click += gcnew System::EventHandler(this, &IncomesUserControl::addButton_Click);
			// 
			// saveButton
			// 
			...
			this->saveButton->Click += gcnew System::EventHandler(this, &IncomesUserControl::saveButton_Click);
			// 
			// dateTimePicker
			// 
			...
			this->dateTimePicker->Size = System::Drawing::Size(385, 20);
			this->dateTimePicker->TabIndex = 1;
			this->dateTimePicker->Visible = false;
			// 
			// amountAdd
			// 
			this->amountAdd->Location = System::Drawing::Point(398, 28);
			this->amountAdd->Name = L"amountAdd";
			this->amountAdd->Size = System::Drawing::Size(385, 20);
			...
			this->filterTextBox->TextChanged += gcnew System::EventHandler(this, &IncomesUserControl::filterTextBox_TextChanged);
			// 
			// ID
			// 
			...
			this->Name = L"IncomesUserControl";
...
#pragma region Events
	...
	private: System::Void addButton_Click(System::Object^  sender, System::EventArgs^  e) {
				 if (dataGridView -> ReadOnly)
				 {
					 ...
					 else
					 {
						 ...
						 // Validate Input
						 if(validateInputsAdd(amountAdd -> Text))
						 {
							 float amount;
							 ...
							 // Auto save Income Controller
							 id = incomeController -> add(tm, amount);
							 autoSave();


							 // Add row to data grid view
							 int newRowIndex = dataGridView -> Rows -> Add();
							 setDataGridRowColumns(dataGridView -> Rows[newRowIndex], id, dateTimePicker -> Value.ToString("MMMM, yyyy"), amount);
				 ...
				 {
					 FormHelper::showErrorMessage("Edit Mode Active","Cannot add income while edit mode is active.");
				 }
			 }
				 ...
				 if (!dataGridView ->ReadOnly)
				 {
					 ...
					 {		
						 ...
						 if (!System::String::IsNullOrWhiteSpace(cellValue) && DateTime::TryParse(cellValue, datetime))
						 {
							 ...
							 incomeController->modifyEffectiveDate(Int32::Parse(dataGridView -> Rows[e->RowIndex] -> Cells["ID"] -> Value ->ToString()), tm);
					 ...
					 else if (columnName  == "Amount")
					 {
						 if (!System::String::IsNullOrWhiteSpace(cellValue) && System::Text::RegularExpressions::Regex::IsMatch(cellValue, "^[$]?\\s?[0-9]{1}[0-9]*([.][0-9]{2})?$"))
						 {
							 ...
							 incomeController->modifyAmount(Int32::Parse(dataGridView -> Rows[e->RowIndex] -> Cells["ID"] -> Value ->ToString()), amount);
							 // Format data cell
						 ...
						 else
						 {
							 dataGridView -> Rows[e->RowIndex] -> Cells[e->ColumnIndex] -> Value = 
								 convertToSystemString(convertToCurrencyString(incomeController->getAmount(Int32::Parse(dataGridView -> Rows[e->RowIndex] -> Cells["ID"] -> Value ->ToString())))); 
	...
	private: System::Void dataGridView_CellContentClick(System::Object^  sender, System::Windows::Forms::DataGridViewCellEventArgs^  e) {
				 ...
				 {
					 incomeController -> remove(Int32::Parse(dataGridView -> Rows[e->RowIndex] -> Cells["ID"] -> Value ->ToString()));
	...
	private: System::Void filterTextBox_TextChanged(System::Object^  sender, System::EventArgs^  e) {
				 ...
				 timer -> Elapsed += gcnew System::Timers::ElapsedEventHandler(this, &IncomesUserControl::filterDataGridView);
				 timer -> Enabled = true;
...
#pragma region Private Methods
	private:
		bool validateInputsAdd(System::String^ amount)
		{
			int errors = 0;
		...
		void setDataGridRowColumns(DataGridViewRow^ row, int id, std::string effectiveDate, float amount)
		{
		...
		void setDataGridRowColumns(DataGridViewRow^ row, int id, System::String^ effectiveDate, float amount)
		{
			row -> Cells["ID"] -> Value = id.ToString();
...
#pragma region Public Methods
	...
	public: bool closeUserControl()
			{
				if (incomeController -> backupExists())
				{
					if(FormHelper::showYesNoMessage("Incomes Not Saved!","Incomes for profile " + convertToSystemString(incomeController -> getProfileName()) + " have not been saved. Do you wish to save changes and exit?"))
					{
						save();
						return true;
					}
					...
					{
						if(FormHelper::showYesNoMessage("Continue Exit","Do you want to discared changes for " + convertToSystemString(incomeController -> getProfileName()) + "'s Incomes and exit the window?"))
						{
							incomeController -> deleteBackup();
							return true;
						}
	...
	void KeyDown(System::Windows::Forms::KeyEventArgs^  e) {
				 ...
				 else if ((this->ActiveControl == amountAdd || this->ActiveControl == dateTimePicker) && e->KeyCode == Keys::Enter && !this->CanFocus)
				 {
